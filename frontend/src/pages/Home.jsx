...
const Homepage = () => {
  ...
  const [buyerData, setBuyerData] = useState({
    buyer: '', buyerPhone: '', price: 0, buyerAddress: ''
  });
  ...
  const calculateStats = () => {
    ...
    const totalValue = phones.reduce((sum, phone) => sum + (phone.sold ? phone.soldPrice : phone.expectedSellingPrice), 0);
  ...
  const handleMarkAsSold = async (phoneId) => {
    try {
      ...
      setBuyerData({ buyer: '', buyerPhone: '', price: 0, buyerAddress: '' });
      ...
      <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={() => {
              setShowBuyerForm(false);
              setBuyerData({ buyer: '', buyerPhone: '', buyerEmail: '', buyerAddress: '' });
            }}
